...
import {
  getAITrips,
  deleteAITrip,
} from "../../features/ai-planner/services/aiTripsStorage";
      ...
      <div className="lg:col-span-2">
        ...
        {loading ? (
          ...
          <div className="rounded-3xl bg-white p-10 text-center shadow-[0_15px_40px_rgba(76,48,24,0.08)] border border-stone-100/50">
            ...
            <Link
              to="/booking"
              className="mt-4 inline-flex rounded-xl bg-[#B8860B] px-6 py-2.5 text-sm font-semibold text-white shadow-md transition hover:bg-[#8B5E3C]"
            >
              Book a Guide Now
            </Link>
          </div>
          ...
          <div className="space-y-6">
            {bookings.map((b) => (
              <BookingCard
                key={b.id}
                booking={b}
                guide={getGuideDetails(b.guideId)}
              />
            ))}
          </div>
        )}
