...
function BookingPage() {
  ...
  useSEO({
    ...
    description:
      "Hire local certified tour guides and Egyptologists for historical tours, desert safari, and Nile cruises in Cairo, Luxor, and Aswan.",
  });
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.fromTo(
        ...
        { opacity: 0, x: -50 },
        { opacity: 1, x: 0, duration: 0.9, delay: 0.2 },
      ).fromTo(
        formRef.current,
  ...
  return (
    <section
      ref={containerRef}
      className="relative min-h-max bg-cover bg-center bg-no-repeat overflow-hidden"
      style={{
      ...
      <div className="relative z-10 mx-auto flex flex-col lg:flex-row min-h-screen max-w-7xl items-center justify-center lg:justify-between gap-12 lg:gap-16 px-6 py-24 lg:pt-32 lg:pb-24 lg:px-10">
        <div ref={leftContentRef} className="max-w-lg text-center lg:text-left">
          ...
          <p className="mt-6 text-base leading-8 text-stone-200 sm:text-lg">
            Book a trusted local guide and discover the beauty of Egypt through
            its temples, the Nile, and authentic Nubian culture.
          </p>

          <div className="mt-8 flex flex-wrap justify-center gap-3.5 lg:justify-start">
            <Link
              to="/"
              className="rounded-2xl border border-white/40 px-6 py-3.5 font-bold text-white transition-all hover:bg-white/20 hover:scale-105 active:scale-95"
            >
              ← Explore with Jawla
            </Link>
            <Link
              to="/profile"
              className="rounded-2xl bg-[#b57a2d] px-6 py-3.5 font-bold text-white shadow-lg transition-all hover:bg-[#8B5E3C] hover:scale-105 active:scale-95"
            >
              View Bookings →
            </Link>
          </div>
        ...
        <div ref={formRef} className="w-full h-fit max-w-150 lg:ml-16">
          <BookingForm />
        </div>
      </div>
    </section>
