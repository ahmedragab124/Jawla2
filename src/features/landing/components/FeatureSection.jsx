  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      currentIndexRef.current =
        (currentIndexRef.current + 1) % destinations.length;
          ...
          {destinations.map((D) => (
            ...
            >
              ...
              <div className="relative z-10 h-full p-7 flex flex-col justify-end text-white text-left">
                <h1 className="text-2xl font-black drop-shadow-md group-hover:text-amber-300 transition-colors duration-300">
                  {D.name}
                </h1>
                <p className="mt-3 text-xs leading-6 text-stone-200 line-clamp-3">
                  {D.description}
                </p>
                <Link
                  to={`/destination/${D.id}`}
